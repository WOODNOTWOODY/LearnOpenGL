...
public:
	...
	*/
	PixelBox(uint32 width, uint32 height, uint32 depth, ElementFormat format, void *pPixData = NULL);
	PixelBox(uint32 left, uint32 top, uint32 right, uint32 bottom, uint32 depth, ElementFormat format, void *pPixData = NULL);
	~PixelBox();
...
public:
	...
	inline uint32			getHeight() const { return bottom - top; }
	// Get the depth of this box
	inline uint32			getDepth() const { return back - front; }
	...
	* mipmap.
	*/
	Color				getColor(uint32 x, uint32 y, uint32 z) const;
	...
	* mipmap.
	*/
	void				setColor(const Color &color, uint32 x, uint32 y, uint32 z);
};
...
public:
	...
	static bool					BulkPixelVerticalFlip(PixelBox& box);
	static void					BulkPixelConversion(void *pSrcData, ElementFormat srcFormat, void *pDestData, ElementFormat dstFormat, uint32 count);
